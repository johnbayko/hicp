...
attributes: <attribute specifiers>
  ...
  "textfield", "textpanel":
    ...
    string, and would look like this::
      ...
      A: The position value allows this to only change a section of the
      content's attributes, so there's no need for a separete "add" action like
      there is for content.
    ...
    Where <position> is the first character the attribute list applies to, the
    ...
    Where <position> is the first character the attribute list applies to,
